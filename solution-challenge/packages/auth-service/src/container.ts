import { createContainer, InjectionMode, AwilixContainer, asValue, asClass } from 'awilix';
...
container.
  loadModules([
    './interface/services/locale-service/locale-service.ts'
  ], {
    cwd: __dirname,
  ...
  //Load loggers
  loadModules(['./interface/utils/logger/logger-factory.ts', './interface/utils/logger/logger-type/*-logger.ts'], {
    cwd: __dirname,
    formatName: 'camelCase',
    resolverOptions: {
      lifetime: 'SINGLETON',
  ...
  loadModules(['./interface/utils/validator/validator.ts', './interface/utils/validator/validator-error-parser.ts'], {
    cwd: __dirname,
    formatName: 'camelCase',
    resolverOptions: {
      lifetime: 'SINGLETON',
  ...
  loadModules([
    './bounded-context/repo/mysql/**/*-repository.ts', './bounded-context/repo/mysql/**/*-mapper.ts', './bounded-context/repo/mysql/base-mysql-repository.ts'], {
    cwd: __dirname,
    formatName: 'camelCase',
    resolverOptions: {
      lifetime: 'SCOPED',
  ...
  loadModules([['./bounded-context/repo/mysql/**/*-repository.ts', <any>{ injector: () => ({ appContext: systemContext }) }], ['./bounded-context/repo/mysql/base-mysql-repository.ts', <any>{ injector: () => ({ appContext: systemContext }) }]], {
    cwd: __dirname,
    formatName: name => {
      // sample format: systemUserRepository
      return `system${chain(name)
        .camelCase()
        .upperFirst()
        .value()}`;
    },
  ...
  loadModules(['./interface/utils/configuration/configuration.ts', './interface/utils/configuration/configuration-mapper.ts'], {
    cwd: __dirname,
    formatName: 'camelCase',
    resolverOptions: {
      lifetime: 'SINGLETON',
  ...
  // Load routes
  loadModules(['./bounded-context/repo/server/routes/*-router.ts',
  ...
  //Load server
  loadModules(['./bounded-context/repo/server/server.ts', './app/application.ts'],
    {
      cwd: __dirname,
      formatName: 'camelCase',
      resolverOptions: {
...
if (process.env.NODE_ENV === 'development') { console.log(`module loaded: `, JSON.stringify(container.registrations)) }
